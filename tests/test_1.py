...
def test_rgi_protein_sequence(rgi):
    ...
    assert validate_results(output_file, 100, 'NDM-1', 'Perfect')


def test_rgi_protein_sequence_tetM(rgi):
    filename = "tetM.fasta"
    output_file = os.path.join(working_directory, outputs, "{}.json".format(filename))
    run_rgi(rgi, 'protein', os.path.join(working_directory, inputs, filename), output_file)
...
def test_rgi_nucleotide_sequence_transposon_tetM(rgi):
...
def test_rgi_nucleotide_sequence(rgi):
    ...
    assert validate_results(output_file, 100, 'NDM-1', 'Perfect')
    assert validate_results(output_file, 100, "APH(3')-VI", 'Perfect')
    assert validate_results(output_file, 100, 'mphE', 'Perfect')
    assert validate_results(output_file, 100, 'msrE', 'Perfect')
...
def test_rgi_variant_model(rgi):
    ...
    assert validate_results(output_file, 99.88, 'Escherichia coli gyrB conferring resistance to aminocoumarin',
                            'Strict')
...
def test_rgi_overexpression_model(rgi):
    ...
    assert validate_results(output_file, 100, 'nalC', 'Strict')
    assert validate_results(output_file, 99.53, 'nalC', 'Strict')
...
def test_rgi_effluxpump_model(rgi):
    ...
    assert validate_results(output_file, 100, 'MexA', 'Perfect')
    assert validate_results(output_file, 100, 'MexB', 'Perfect')
    assert validate_results(output_file, 100, 'OprM', 'Perfect')
    assert validate_results(output_file, 100, 'Pseudomonas aeruginosa CpxR', 'Perfect')
    assert validate_results(output_file, 99.82, 'MexB', 'Strict')
    assert validate_results(output_file, 99.59, 'OprM', 'Strict')
    assert validate_results(output_file, 98.12, 'nalC', 'Strict')
    assert validate_results(output_file, 97.28, 'MexR', 'Strict')
    assert validate_results(output_file, 89.12, 'MexR', 'Loose')
...
def test_rgi_nudge_loose_to_strict(rgi):
    ...
    assert validate_results(output_file, 98.06, 'Escherichia coli EF-Tu mutants conferring resistance to Pulvomycin',
                            'Strict')
