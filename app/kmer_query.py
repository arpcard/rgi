...
class CARDkmers(object):
    ...
    def __init__(self, input, bwt, rgi, fasta, k, min, threads, output, local, debug):
        ...
        self.output_json_file = os.path.join(
            self.working_directory, "{o}_{k}mer_analysis.json".format(o=self.output, k=self.k))
        if self.bwt:
            self.input_bam_file = input
            self.bam_directory = os.path.dirname(self.input_bam_file)
            self.output_allele_summary = os.path.join(
                self.working_directory, "{o}_{k}mer_analysis.allele.txt".format(o=self.output, k=self.k))
            self.output_gene_summary = os.path.join(
                self.working_directory, "{o}_{k}mer_analysis.gene.txt".format(o=self.output, k=self.k))
        if self.rgi:
            self.input_rgi_file = input
            self.output_rgi_summary = os.path.join(
                self.working_directory, "{o}_{k}mer_analysis_rgi_summary.txt".format(o=self.output, k=self.k))
        if self.fasta:
            self.input_fasta_file = input
            self.output_fasta_summary = os.path.join(
                self.working_directory, "{o}_{k}mer_analysis_fasta_summary.txt".format(o=self.output, k=self.k))
            ...
            try:
                for key, value in rgi_data.items():
                    if isinstance(value, dict):
                        ...
                        hsp = max(value.keys(), key=(
                            lambda key: value[key]["bit_score"]))
    ...
    def get_bwt_alignment_data(self, header):
        ...
        if flag.isdigit() is False:
            logger.error("failed to parse BAM file: {}, please check which aligner software was used to produce the BAM file in the RGI BWT step.".format(
                self.input_bam_file))
            exit()
        else:
            if int(flag) & 64:
    ...
    def split_fasta(self, file):
        ...
        # counts number of sequences in generator
        ns = sum(1 for i in temp_iterator)
        # maximizes list size for threads available
        list_size = math.ceil(ns/self.threads)
        split_sequences = list(self.chunk_list(iterator, list_size)) if list_size > 0 else [
            []]  # returns a list of lists
        logger.info("Using {t} threads to query {s} sequences".format(
            t=self.threads, s=ns))
    ...
    def query_sequences(self, k, j, amr_kmers, fasta, type, *args):
        ...
        for entry in fasta:
            ...
            if num_kmers > 1:
                ...
                gen = OrderedDict(
                    {'chr + plasmid': bc, 'plasmid': pc, 'chr': cc})
                ...
                if all(not tax[k] for k in tax) and all(v == 0 for v in gen.values()):
                    ...
                    if type == "rgi":
                        ...
                        self.populate_fasta_json(
                            read, num_kmers, amr_c, tax, gen, o)
                    else:
                        logger.error("incompatible input type")
            else:
                short += 1
    ...
    def get_taxon_data(self, name, d, tax):
        if name not in d:
            d[name] = {}
    ...
    def make_summaries(self, d, ssc, sscp, ssp, ssu, sgc, sgcp, sgp, sgu, cd, m, cp, ad):
        ...
        for a in d:
            ...
            if a in ssc:
                ssc_str, ssc_hits, ssc_pred_str = self.organize_summary_data(
                    a, ssc, 0)
            else:
            ...
            if a in sscp:
                ...
                sscp_hits = 0
            if a in ssp:
                ssp_str, ssp_hits, ssp_pred_str = self.organize_summary_data(
                    a, ssp, 2)
            else:
                ssp_str, ssp_pred_str = "", ""
                ssp_hits = 0
            if a in ssu:
                ssu_str, ssu_hits, ssu_pred_str = self.organize_summary_data(
                    a, ssu, 3)
            else:
                ssu_str, ssu_pred_str = "", ""
                ssu_hits = 0
            if a in sgc:
                sgc_str, sgc_hits, sgc_pred_str = self.organize_summary_data(
                    a, sgc, 0)
            else:
            ...
            if a in sgcp:
                ...
                sgcp_hits = 0
            if a in sgp:
                sgp_str, sgp_hits, sgp_pred_str = self.organize_summary_data(
                    a, sgp, 2)
            else:
                sgp_str, sgp_pred_str = "", ""
                sgp_hits = 0
            if a in sgu:
                sgu_str, sgu_hits, sgu_pred_str = self.organize_summary_data(
                    a, sgu, 3)
            else:
            ...
            if a in ad:
                ...
                ambc = 0
            total_hits = ssc_hits + sscp_hits + ssp_hits + ssu_hits + \
                sgc_hits + sgcp_hits + sgp_hits + sgu_hits + mc + cpc + ambc + cc

            prediction_str = ssc_pred_str + sscp_pred_str + ssp_pred_str + \
                ssu_pred_str + sgc_pred_str + sgcp_pred_str + sgp_pred_str + sgu_pred_str
            ...
            if j[read]['#_of_kmers_in_sequence'] > 0:
                ...
                if j[read]['#_of_kmers_in_sequence'] > 0:
                    ...
                    if len(j[read]['taxonomic_info']['species']) == 1:
                        path = set(j[read]['taxonomic_info']
                                   ['species'].keys()).pop()
                            ...
                            if j[read]['taxonomic_info']['species'][path] > self.min-1:
                                ...
                                if type == "bwt":
                                    self.single_species_bwt(
                                        j[read], path, allele, ssc_allele, ssp_allele, sscp_allele, ssu_allele)
                                else:
                                ...
                                if type == "bwt":
                                    self.ambiguous_bwt(
                                        j[read], allele, cp_allele, a_allele, m_allele, c_allele, False)
                                else:
                                    prediction = self.ambiguous_rgi(
                                        j[read], False)
                        else:
                            if len(j[read]['taxonomic_info']['genus']) == 1:
                                genus = set(j[read]['taxonomic_info']
                                            ['genus'].keys()).pop()
                                if genus == path.split()[0]:
                                    ...
                                    if j[read]['taxonomic_info']['genus'][genus] > self.min-1 and j[read]['taxonomic_info']['species'][path] > self.min-1:
                                        ...
                                        if type == "bwt":
                                            self.ambiguous_bwt(
                                                j[read], allele, cp_allele, a_allele, m_allele, c_allele, True)
                                        else:
                                            prediction = self.ambiguous_rgi(
                                                j[read], True)
                                        ...
                                        if type == "bwt":
                                            self.single_genus_bwt(
                                                j[read], genus, allele, sgcp_allele, sgc_allele, sgp_allele, sgu_allele)
                                        else:
                                ...
                                max_genus = max(j[read]['taxonomic_info']['genus'].keys(), key=(
                        ...
                        hits += 1
                        max_species = max(j[read]['taxonomic_info']['species'].keys(), key=(
                            lambda key: j[read]['taxonomic_info']['species'][key]))
                        # multiple single species
                        if sum(int(c) > self.min-1 for c in j[read]['taxonomic_info']['species'].values()) > 1:
                            passing_species = [path for path in j[read]['taxonomic_info']['species']
                                               if j[read]['taxonomic_info']['species'][path] > self.min-1]
                            genera = [path.split()[0]
                                      for path in passing_species]
                            if len(set(genera)) == 1:
                            ...
                            if not j[read]['taxonomic_info']['genus']:
                                if type == "bwt":
                                    self.single_species_bwt(
                                        j[read], max_species, allele, ssc_allele, ssp_allele, sscp_allele, ssu_allele)
                                else:
                                    prediction = self.single_species_rgi(
                                        j[read], max_species)
                            else:
                                if len(j[read]['taxonomic_info']['genus']) == 1:
                                    genus = set(
                                        j[read]['taxonomic_info']['genus'].keys()).pop()
                            ...
                            if not j[read]['taxonomic_info']['genus']:
                                ...
                                if len(j[read]['taxonomic_info']['genus']) == 1:
                                    ...
                                    if sum(int(c) > self.min-1 for c in j[read]['taxonomic_info']['genus'].values()) > 1:
                                        ...
                                        if type == "bwt":
                                            self.single_genus_bwt(
                                                j[read], max_genus, allele, sgcp_allele, sgc_allele, sgp_allele, sgu_allele)
                                        else:
                                            prediction = self.single_genus_rgi(
                                                j[read], max_genus)
                                    else:
    ...
    def run(self):
        ...
        if self.rgi:
            logger.info("input rgi results file: {}".format(
                self.input_rgi_file))
            ...
            logger.info(
                "input RGI*BWT bam file: {}".format(self.input_bam_file))
            # amos - maybe use pybam to get gname, flag etc (https://github.com/JohnLonginotto/pybam)
            self.get_bwt_sequences()
            # split sequences for threading
            split_sequences = self.split_fasta(self.fasta_file)
            # Threading
            results = self.execute_threads(
                split_sequences, j, amr_kmers, "bwt")
            ...
            # Threading
            results = self.execute_threads(
                split_sequences, j, amr_kmers, "fasta")
        ...
        if self.bwt:
            all_alleles, ssc_allele, ssp_allele, sscp_allele, ssu_allele, sgc_allele, \
                sgp_allele, sgcp_allele, sgu_allele, c_allele, m_allele, cp_allele, a_allele = self.parse_kmer_json(
                    "bwt")
